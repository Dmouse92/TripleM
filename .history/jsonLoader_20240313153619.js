...
function updateOrderNumbers() {
	...
	document.querySelectorAll('.img-wrap').forEach((wrap, index) => {
		...
		if (orderNumber) {
			const newIndex = parseInt(orderNumber.textContent.replace('#', '')) - 1
			orderNumber.textContent = `#${newIndex + 1}`
			const path = wrap.querySelector('.preview-img').src
			imageFiles[newIndex] = path
		}
	})
