...
function updateOrderNumbers() {
	...
	document.querySelectorAll('.img-wrap').forEach((wrap, index) => {
		...
		const orderedImages = Array.from(imageFiles).map((image, index) => {
			const orderNumber = index + 1
			const updatedImage = { ...image, orderNumber }
			return updatedImage
		})
		imageFiles = orderedImages
		const orderedImagePaths = Array.from(imageElements).map((image) => image.path)
		imageFiles = orderedImagePaths
	})
}
