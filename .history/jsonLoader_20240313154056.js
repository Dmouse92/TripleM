...
function updateOrderNumbers() {
	...
	if (imgWraps) {
		Array.from(imgWraps).forEach((wrap, index) => {
		...
		const orderedImages = Array.from(imgWraps)
			.map((imgWrap) => {
				const previewImg = imgWrap.querySelector('.preview-img')
				if (previewImg) {
					const path = previewImg.src
					return path
				}
				return undefined // To avoid undefined values in the array
			})
			.filter((image) => image !== undefined) // Remove undefined values from the array

		imageFiles = orderedImages
	} else {
		console.error('No img-wrap elements found')
	}
}
