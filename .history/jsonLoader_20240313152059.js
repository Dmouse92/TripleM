...
function handleFormSubmit(e) {
	...
	let imagePaths = [] // Array to store the file paths on the backend
	if (imageFiles && imageFiles.every((file) => file && file.name)) {
		imageFiles.forEach((file, index) => {
			...
			imagePaths.push(`/assets/img/vehicles/${make}_${model}_${vin}/${newFilename}`)
		})
	}
	...
	fetch('/upload', {
		...
		.then((data) => {
			// Modify the img key of the JSON file with the array of file paths
			const jsonData = {
				// other data in the JSON file
				img: imagePaths,
			}
