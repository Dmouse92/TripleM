	...
	db.get(
		...
		(err, result) => {
			if (err) {
				...
				let imgs = row.img.split(',') // Assuming 'img' is a valid field in the row object
				const thumbnailImage = imgs[0] // Take the first image for the thumbnail

				// Construct the preview data object
				const previewData = {
					id: row.id,
					imagePath: `/assets/img/vehicles/${thumbnailImage}`, // Assuming thumbnailImage is just the filename
