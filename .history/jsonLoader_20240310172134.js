...
document.getElementById('addForm').addEventListener('submit', function (event) {
	...
	// Get the image URLs from the #preview div
	var imageUrls = []
...
document.getElementById('addForm').addEventListener('submit', function (e) {
	...
	var make = formObject['make']
	var model = formObject['model']
	var vin = formObject['vin']

	var dir = '/assets/img/vehicles/' + make + '_' + model + '_' + vin + '/'

	if (!fs.existsSync(dir)) {
		fs.mkdirSync(dir, { recursive: true })
	}

	for (let i = 0; i < formObject['images'].length; i++) {
		var oldPath = formObject['images'][i]
		var newPath = dir + make + '_' + model + '_' + vin + '_' + (i + 1)
		fs.rename(oldPath, newPath, function (err) {
			if (err) throw err
		})
		formObject['images'][i] = newPath
	}
