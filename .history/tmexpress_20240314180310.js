...
console.log('ORIGINAL FILE PATHS:', uniqueFiles.map((file) => file.path));
)

		// Process images and wait for all to be resized and cropped
		const imageProcessingPromises = uniqueFiles.map((file, index) => {
			const inputPath = file.path
			const outputPath = path.join(
				dir,
				`${make}_${model}_${vin}_${index}${path.extname(file.originalname)}`
			)
			console.log('Resizing: ', inputPath)
			return resizeAndCropImage(inputPath, outputPath).then(() => outputPath)
		})
