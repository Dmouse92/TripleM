...
function orderUpdateNumbers(imageElements) {
	const orderedImages = Array.from(imageElements).map((image, index) => {
		const orderNumber = index + 1
		// A: Re-render the images in img-wrap with new index indicators
		renderImageWithIndexIndicator(image, orderNumber)
		return image.path // B: create a new 0-based array with image paths
	})
...
function handleFormSubmit(e) {
	...
	if (imageFiles && imageFiles.every((file) => file && file.name)) {
		imageFiles
			.sort((a, b) => a.name.localeCompare(b.name)) // Sort the image files by name
	...
	formData.append('order', JSON.stringify(imageFiles.map((file) => file.name)))
