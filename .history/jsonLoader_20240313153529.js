...
function updateOrderNumbers() {
	// Re-order the imageFiles array without changing the file references
	...
	document.querySelectorAll('.img-wrap').forEach((wrap, index) => {
		...
		if (orderNumber) {
			const originalIndex = parseInt(orderNumber.textContent.replace('#', '')) - 1
			orderedImageFiles[index] = imageFiles[originalIndex]
		}
	})
	imageFiles = orderedImageFiles // Update imageFiles with the re-ordered array
}
