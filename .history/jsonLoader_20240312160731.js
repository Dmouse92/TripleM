...
function handleFormSubmit(e) {
	...
	// Append each image file to formData only once
	imageFiles.forEach((file, index) => {
		formData.append('images', file)
	})

	submitFormData(formData) // Proceed to send formData to the server
}

// Submit form data
function submitFormData(formData) {
	// Fetch request to the server
	console.log( JSON.stringify(imageFiles.map((file, index) => index)), // Send order as a header)
	// Fetch request to the server
	fetch('http://localhost:3000/upload', {
		method: 'POST',
		body: formData,
		headers: {
			'Order-Index': JSON.stringify(imageFiles.map((file, index) => index)), // Send order as a header
