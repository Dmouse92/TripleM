...
app.post('/preview', (req, res) => {
	// Extract data from the request, e.g., an ID
	const { id } = req.body // You might be sending an ID in the form data

	// Retrieve listing information based on the ID
	// This is pseudocode and depends on your data storage solution (e.g., SQL, NoSQL, etc.)
	db.get('SELECT * FROM listings WHERE id = ?', [id], (err, result) => {
		if (err) {
		...
		if (result.length > 0) {
			const listingData = result[0] // Assuming the query returns an array of listings

			// Construct the preview data object
			const previewData = {
				id: listingData.id,
				imagePath: `/path/to/images/${listingData.id}`, // Construct the path as needed
				// ... include other necessary data for preview
			}
...
function deleteImageDirectories(listingId) {
	// Implement the logic to delete the image directories associated with the listing
	// This is just a placeholder function and should be replaced with actual logic
	console.log(`Delete image directories for listing id: ${listingId}`)
	// Return true if deletion was successful, false otherwise
	return true
}
// tmexpress.js: Route to handle individual car details
app.get('/preview/cars/:make/:model/:vin', (req, res) => {
	// Extract make, model, and vin from the request parameters
	const { make, model, vin } = req.params
