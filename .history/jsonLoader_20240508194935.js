...
function toggleAddCarForm(show) {
	...
	if (show) {
		...
		const pond = FilePond.create(inputElement, {
			...
			allowReorder: true,
			imagePreviewHeight: 140, // Set the height of the preview
			imagePreviewWidth: 'auto', // Set the width of the preview
			imagePreviewMaxFileSize: '5MB', // Set the maximum file size for the preview
		...
		pond.on('reorderfiles', (files) => {
			...
			files.forEach((file, index) => {
				// Update or set metadata for order
				pond.setMetadata(file.id, { orderIndex: index + 1 })

				// Optionally, update the UI here if you have visual indices displayed
				// This part depends on your specific UI implementation
			})
		})
