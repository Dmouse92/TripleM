	...
	db.get(
		...
		(err, row) => {
			if (err) {
				...
				// Assume row.img contains a comma-separated list of image file names
				const imageFiles = row.img ? row.img.split(',') : []
				const imageUrls = imageFiles.map((fileName, index) => {
					// Construct the URL, assuming images are sorted by file name
					return `http://<your-image-path>/${fileName}`
				})

				// Render the EJS page with the car data and image URLs
				res.render('<ejs-template-name>', { car: row, images: imageUrls })
			} else {
