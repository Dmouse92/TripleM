		...
		sharp(inputPath)
			...
			.then((buffer) => {
				fs.promises
					...
					.then(() => {
						console.log('Image processed and written successfully:', outputPath)
						scheduleFileDeletion(inputPath) // Schedule deletion after processing is confirmed complete
						resolve(outputPath)
					})
					.catch((error) => {
						console.error('Error writing processed image:', error)
						reject(error)
					})
			})
			.catch((error) => {
...
app.post('/upload', upload.array('images'), async (req, res) => {
	...
	try {
		...
		const imgPath = uniqueFiles.map((file) => file.path).join(',')
		db.serialize(() => {
				...
				getImageCountForCar(make, model, vin)
					.then((numberOfImages) => {
						...
						for (let i = 0; i < numberOfImages; i++) {
							imageUrls.push(`${basePath}image_${i}.jpg`)
						}
