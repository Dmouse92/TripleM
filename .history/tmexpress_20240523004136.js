...
const db = new betterSqlite3(path.join(__dirname, 'vehdatabase.sqlite'))
...
app.get('/cars/:make/:model/:vin', (req, res) => {
	...
	console.log(`Received request for car details: make=${make}, model=${model}, vin=${vin}`)

	try {
		const stmt = db.prepare('SELECT * FROM vehDB WHERE make = ? AND model = ? AND vin = ?')
		const car = stmt.get(make, model, vin)
