...
app.post('/upload', upload.array('images'), (req, res) => {
	...
	try {
		...
		*/
		...
		// Sort the req.files array based on the order
		req.files.forEach((file, index) => {
			if (!/\d+/.test(file.fieldname)) {
				console.error(`File at index ${index} does not have index in fieldname: ${file.fieldname}`)
			}
		})

		req.files.sort((a, b) => {
			const matchA = a.fieldname.match(/\d+/)
			const matchB = b.fieldname.match(/\d+/)
			if (matchA && matchB) {
		...
		console.log('O INDEX: ', orderIndex)
		console.log('O2 INDEX: ', order2)
