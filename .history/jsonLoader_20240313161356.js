...
function handleFormSubmit(e) {
	...
	if (imageFiles && imageFiles.every((file) => file && file.name)) {
		imageFiles.forEach((file, index) => {
			...
			if (!a) {
				console.error('Failed to create a.')
				return
			}
			a.href = url
			if (!a.href) {
				console.error('Failed to set a.href.')
				return
			}
			a.download = newFilename
			if (!a.download) {
				console.error('Failed to set a.download.')
				return
			}
			a.click()
			if (!url) {
				console.error('Failed to create ObjectURL.')
				return
			}
			URL.revokeObjectURL(url)
			if (!formData) {
				console.error('Failed to get formData.')
				return
			}
