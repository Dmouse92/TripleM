...
const db = new sqlite3.Database('vehdatabase.db', (err) => {
	if (err) {
...
db.run('CREATE TABLE IF NOT EXISTS vehDB (    make TEXT,
    model TEXT,
    year INTEGER,
    price INTEGER,
    vin TEXT,
    ...
    comments TEXT,
    img TEXT,
    PRIMARY KEY (make, model, vin))', (err) => {
	if (err) {
...
app.post('/upload', upload.array('images'), async (req, res) => {
	...
	try {
		...
		const newCar = {
			...
			exterior,
			interior,
			fuelEconomy: fuelEconomy.split(',').map((fuelEconomy) => fuelEconomy.trim()),
