...
app.use(
	cors({
		origin: 'http://127.0.0.1:3000', // Or use '*' to allow all origins
	...
	db.get(
		...
		(err, row) => {
			if (err) {
				...
				// Render the EJS page with the car data
				res.render('product', { car: row })
			} else {
...
db.get(    'SELECT * FROM vehDB WHERE make = ? AND model = ? AND vin = ?',    [make, model, vin], change this to grtab all attributes, and have it also quickly build an array of file urls for the images called img, use the suffix of images to sort from 0->X and send that with it REATE TABLE "vehDB" (   make TEXT,   model TEXT,   year INTEGER,   price INTEGER,   vin TEXT,   engine TEXT,   transmission TEXT,   drive TEXT,   mileage INTEGER,   exterior TEXT,   interior TEXT,   fuelEconomy TEXT,   features TEXT,   comments TEXT,   PRIMARY KEY (make, model, vin)