...
function toggleAddCarForm(show) {
	...
	if (show) {
		...
		const imageResizeOptions = {
			mode: 'cover', // 'force', 'contain', 'cover', or 'scale-down'
			width: 1024,
			height: 768,
			upscale: false,
		}
		...
		const pond = FilePond.create(inputElement, {
			...
			server: false,
			imageResizeTargetWidth: imageResizeOptions.width,
			imageResizeTargetHeight: imageResizeOptions.height,
			imageResizeMode: imageResizeOptions.mode,
			imageResizeUpscale: imageResizeOptions.upscale,
			imageEditEditor: FilePondPluginImageEdit.createEditor({}),
