...
app.post('/upload', upload.array('images'), (req, res) => {
	try {
		...
		// Loop through the files only once
  req.files.forEach((file, i) => {
    // Construct the new path for the file
    const newFilePath = // ... code to generate file path with index
    
    // Check if the file already exists to avoid duplicates
    if (!fs.existsSync(newFilePath)) {
      // Move the file to the newFilePath only if it does not exist
      fs.renameSync(file.path, newFilePath);
		...
		req.files.forEach((file, i) => {
			...
			const newFileName = `${make}_${model}_${vin}_${newPosition}${path.extname(file.originalname)}`
