...
app.post('/upload', upload.array('images'), (req, res) => {
	...
	try {
		...
		// Now use the resizeAndCropImage function on each unique file
		// Now use the resizeAndCropImage function on each unique file
		uniqueFiles.forEach((file) => {
			const inputPath = file.path // The path where the uploaded file is stored
			const outputPath = inputPath // Overwrite the original file

			// Call the resize and crop function
			resizeAndCropImage(inputPath, outputPath)
		})
...
// Function to resize and crop image to 1024x1024 and move to a specific directory
function resizeAndCropImage(inputPath, make, model, vin) {
	// Sanitize make, model, and vin to be used in file paths
	const safeMake = make.replace(/[^a-zA-Z0-9]/g, '_')
	const safeModel = model.replace(/[^a-zA-Z0-9]/g, '_')
	const safeVin = vin.replace(/[^a-zA-Z0-9]/g, '_')

	// Construct the new path
	const newDirectory = `assets/img/vehicles/${safeMake}_${safeModel}_${safeVin}`
	const filename = path.basename(inputPath)
	const outputPath = path.join(newDirectory, filename)
	...
	return new Promise((resolve, reject) => {
		sharp(inputPath)
			...
			.toFile(outputPath, (err, info) => {
				if (err) {
					console.error('Error resizing and moving image:', err)
					reject(err)
				} else {
					console.log('Image resized and moved to:', outputPath)
					resolve(outputPath) // Resolve with the new output path
				}
			})
	})
}
