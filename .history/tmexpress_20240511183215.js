...
const storage = multer.diskStorage({
	destination: processAndCheckDuplication,
	filename: function (req, file, cb) {
		cb(null, path.basename(file.path)) // Extract the filename from the file's path
	},
})
// Middleware to process and check duplication before saving with Multer
async function processAndCheckDuplication(req, file, cb) {
	try {
		...
		// Step 5: Process the image and resize/crop as necessary
		const processedImagePath = await resizeAndCropImage(file.path, outputPath)

		// Step 6: Check for duplication using the image count
		const imageCount = await getImageCountForCar(make, model, vin)

		// Use image count to create a new filename if necessary
		const newFilename = `image${imageCount}.jpg`
		const finalPath = path.join(dir, newFilename)

		// Rename the processed image to follow the naming convention
		fs.renameSync(processedImagePath, finalPath)

		// Set the new path on the file object so Multer can use it
		file.path = finalPath
		cb(null, dir)
	} catch (error) {
		console.error('Error in pre-processing or duplication check:', error)
		cb(error)
	}
}
...
app.post('/upload', upload.array('images'), async (req, res) => {
	...
	try {
		...
		const imageProcessingPromises = uniqueFiles.map((file, index) => {
			...
			//const outputPath = path.join(path.dirname(file.path), path.basename(file.path)) // Keep the original file path for output
			...
			const suffixedFileName = `${originalFileName.split('.')[0]}${dummySuffix}.jpg` // Add dummy suffix to the original file name
			const suffixedFilePath = path.join(path.dirname(file.path), suffixedFileName) // Create path with the suffixed file name

			const outputPath = path.join(path.dirname(file.path), `${originalFileName.split('.')[0]}.jpg`) // Use the original file path for resizing
			/*
			...
			const tmpFilePath = path.join(path.dirname(file.path), `tmp_${path.basename(file.path)}`)

			// Copy the original file to the temporary path
			return fs.promises
				.copyFile(inputPath, tmpFilePath)
				.then(() => resizeAndCropImage(tmpFilePath, outputPath)) // Resize and crop the temporary image
				.then(() => setTimeoutPromise(100)) // Wait for 100 ms before trying to unlink
				.then(() => fs.promises.unlink(tmpFilePath)) // Delete the temporary file
				.then(() => outputPath) // Return the path of the processed image
				.catch((err) => {
		...
		;(async () => {
			try {
				...
				// Use the processedImages array as needed
			} catch (err) {
