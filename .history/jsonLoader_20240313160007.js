...
function updateOrderNumbers() {
	...
	document.querySelectorAll('.img-wrap').forEach((wrap, index) => {
		...
		if (orderNumber) {
			...
			const oldPos = parseInt(orderNumber.textContent) - 1 // Get the current order number of
			orderNumber.textContent = `#${oldPos}` // Update the order number in the top left corner
		}
