...
app.get('/cars/:make/:model/:vin', (req, res) => {
	...
	db.get(
		'SELECT *, GROUP_CONCAT(img) AS img FROM vehDB WHERE make = ? AND model = ? AND vin = ?',
		[make, model, vin],
		(err, row) => {
			if (err) {
				...
				// Process the img field to create an array of image URLs
				let imageUrls = []
				if (row.img) {
					// Split the img field into an array and sort by the suffix number
					imageUrls = row.img
						.split(',')
						.map((img) => img.trim()) // Remove any extra whitespace
						.sort((a, b) => {
							// Extract suffix number and sort numerically
							let aSuffix = parseInt(a.match(/(\d+)\.[^.]+$/)[1], 10)
							let bSuffix = parseInt(b.match(/(\d+)\.[^.]+$/)[1], 10)
							return aSuffix - bSuffix
						})
				}

				// Send the car data along with the sorted array of image URLs
				res.json({ car: row, images: imageUrls })
			} else {
