	...
	let formData = new FormData(e.target)
			...
			if (imageFiles && imageFiles.every((file) => file && file.name)) {
				imageFiles.forEach((file, index) => {
					...
					const newFilename = `${make}_${model}_${vin}_${index}.${extension}` // Update this line to construct the correct file path
					formData.append('images', file, newFilename) // Append the image file to formData with the correct name
				})
			} else {
				console.error('Failed to validate imageFiles.')
				return
			}
