...
app.post('/upload', upload.array('images'), (req, res) => {
	...
	try {
		...
		uniqueFiles.forEach((file) => {
			const inputPath = file.path // Assuming 'file.path' is the path where the uploaded file is stored
			const filenameWithoutExt = path.basename(file.originalname, path.extname(file.originalname))
			const outputPath = path.join(dir, `${filenameWithoutExt}z_${path.extname(file.originalname)}`)
...
// Function to resize and crop image to 1024x1024
