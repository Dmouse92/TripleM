...
app.post('/upload', upload.array('images'), (req, res) => {
	...
	var dir = path.join('/assets/img/vehicles/', make + '_' + model + '_' + vin)
	...
	for (let i = 0; i < req.files.length; i++) {
		...
		var newPath = path.join(dir, make + '_' + model + '_' + vin + '_' + (i + 1) + extension)
	...
	var newCar = {
		...
		features: features,
		comments: comments,
		img: req.files.map((file) => file.path), // Include the paths of the uploaded images
	}
